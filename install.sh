...
选项:
  ...
  --repo <owner/repo> 覆盖仓库地址，默认 manateelazycat/cctui
  -y, --yes           跳过确认提示
  -h, --help          显示帮助
...
github_api() {
  local path="$1"
  local url="${MIRROR:+${MIRROR}/}https://api.github.com${path}"
  curl -fsSL -H "Accept: application/vnd.github.v3+json" "$url" 2>/dev/null
}

github_download() {
  local path="$1" output="$2"
  local url="${MIRROR:+${MIRROR}/}https://github.com${path}"
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ -z "$VERSION" ]]; then
    if [[ "$FORCE_BUILD" != "true" ]] && ! command -v go &>/dev/null; then
      die "无法获取最新版本（网络问题？），请用 --version 手动指定或检查网络"
    fi
...
build_from_source() {
  ...
  if ! command -v go &>/dev/null; then
    die "未找到 Go 编译器。请先安装 Go 1.21+：https://go.dev/dl/"
  fi
